...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getUserFavorites(session.user.id, parsed.data);
    return NextResponse.json({ success: true, data });
  } catch (err) {
    console.error('Error fetching favorites API:', err);
    return NextResponse.json(
      { success: false, error: 'INTERNAL_ERROR', message: 'Lỗi khi tải danh sách yêu thích.' },
      { status: 500 }
    );
  }
}
